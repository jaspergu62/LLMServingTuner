...
```
from llmservingtuner.config.config import Settings
...
def register():
    ...
    from llmservingtuner.config.config import register_settings
...
- 继承llmservingtuner.optimizer.simulator.SimulatorInterface,实现base_url 和 data_field property,实现update_command方法等。
例如：
```
...
- 注册服务框架
```
 from llmservingtuner.optimizer.register import register_simulator
 register_simulator("vllm_infer", VllmSimulator)
```
### 自定义性能测试benchmark

- 继承llmservingtuner.optimizer.benchmark.BenchmarkInterface,实现data_field property, get_performance_index方法等。
例如：
``` 
...
- 注册benchmark
```
from llmservingtuner.optimizer.register import register_benchmarks
register_benchmarks("vllm_infer_benchmark", VllmBenchMark)
```
## 三. 设置插件入口点
将自定义的内容的注册函数添加到入口组'llmservingtuner.plugins'即可。
...
常用数据结构定义
- llmservingtuner.config.config.OptimizerConfigField
```
class llmservingtuner.config.config.OptimizerConfigField(*, name: str = 'max_batch_size', config_position: str = 'BackendConfig.ScheduleConfig.maxBatchSize', min: float = 0.0, max: float = 100.0, dtype: str = 'float', value: int | float | bool | None = None, dtype_param: Any = None)
    Bases: BaseModel
    寻优参数的结构定义。    
...
```
- llmservingtuner.config.config.PerformanceIndex
```
class llmservingtuner.config.config.PerformanceIndex(*, generate_speed: float | None = None, time_to_first_token: float | None = None, time_per_output_token: float | None = None, success_rate: float | None = None, throughput: float | None = None)
...
```
### 配置自定义
- llmservingtuner.config.config.register_settings
```
llmservingtuner.config.config.register_settings(func: Callable | None = None) → None
...
### benchmark 接口
```
class llmservingtuner.optimizer.benchmark.BenchmarkInterface()
    Bases: ABC
    ...
    abstract update_command() → None
        ...
        Returns: None
```
注册实现：llmservingtuner.optimizer.register.register_benchmarks

### 框架服务操作接口
```
class llmservingtuner.optimizer.simulator.SimulatorInterface()
    Bases: ABC
