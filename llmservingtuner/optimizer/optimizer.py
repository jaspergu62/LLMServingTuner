...
from llmservingtuner.common import is_vllm, is_mindie
from llmservingtuner.config.base_config import (
...
from llmservingtuner.optimizer.register import benchmarks, simulates
from llmservingtuner.optimizer.performance_tunner import PerformanceTuner
from llmservingtuner.optimizer.utils import get_required_field_from_json, is_root
...
class PSOOptimizer(PerformanceTuner):
    def __init__(self, scheduler, n_particles: int = 10, iters=100, pso_options=None,
                 ...
                 cbo_enabled: bool = True, constraint_thresholds: Optional[Dict] = None,
                 **kwargs):
        from llmservingtuner.config.config import PsoOptions, default_support_field
        ...
        # CBO surrogate model configuration
        self.cbo_enabled = cbo_enabled
    ...
    def computer_fitness(self) -> Tuple:
        from llmservingtuner.config.config import PerformanceIndex, field_to_param
        all_position = []
        all_cost = []
    ...
    def op_func(self, x) -> np.ndarray:
        ...
        When CBO is enabled, uses surrogate model to filter candidates before real evaluation.
        """
        n_particles = x.shape[0]
        logger.debug(f"Acquired n_particles: {n_particles}, value: {x}")
        generate_speed = []

        # Determine which particles to evaluate based on CBO filtering
        if self.cbo_enabled and self.cbo_model is not None:
            try:
                import torch
                x_tensor = torch.tensor(x, dtype=torch.float64)
                selected_indices, predictions = self.cbo_model.select_candidates(x_tensor)
                logger.debug(f"CBO selected {len(selected_indices)}/{n_particles} candidates for real evaluation")
            except Exception as e:
                logger.warning(f"CBO candidate selection failed: {e}. Evaluating all particles.")
        ...
        for i in range(n_particles):
            if i in selected_indices:
                ...
                try:
                    ...
                    # Add observation to CBO model if enabled
                    if self.cbo_enabled and self.cbo_model is not None:
                        try:
                            import torch
                            x_obs = torch.tensor(x[i:i+1], dtype=torch.float64)
                            y_obs = torch.tensor([[
                                _res.generate_speed or 0,
                                _res.time_to_first_token or inf,
                                _res.time_per_output_token or inf,
                                _res.throughput or 0
                            ]], dtype=torch.float64)
                            self.cbo_model.add_real_observation(x_obs, y_obs)
                        except Exception as e:
                            logger.warning(f"Failed to add CBO observation: {e}")

                except Exception as e:
                    logger.error(f"Failed. error: {e}, please check.")
                    _fitness = inf
            else:
                # Use surrogate prediction (particle filtered out by CBO)
                if predictions is not None:
                    try:
                        pred_mean, _ = predictions
                        pred_speed = pred_mean[i, 0].item()
                        pred_ttft = pred_mean[i, 1].item()
                        pred_tpot = pred_mean[i, 2].item()
                        # Create a dummy result from prediction
                        from llmservingtuner.config.config import PerformanceIndex
                        _res = PerformanceIndex(
                            generate_speed=pred_speed,
                            time_to_first_token=pred_ttft,
                            time_per_output_token=pred_tpot,
                            success_rate=1.0
                        )
                        _fitness = self.minimum_algorithm(_res)
                        # Save as dummy/predicted result
                        self.scheduler.save_dummy_result(fitness=_fitness, is_predicted=True)
                        generate_speed.append(_fitness)
                        continue
                    except Exception as e:
                        logger.warning(f"Failed to use prediction for particle {i}: {e}")
                # Fallback to real evaluation if prediction fails
                try:
                    _res = self.scheduler.run_with_request_rate(x[i], self.target_field)
                    _fitness = self.minimum_algorithm(_res)
                except Exception as e:
        ...
        # Update CBO model after each iteration
        if self.cbo_enabled and self.cbo_model is not None:
            try:
                self.cbo_model.update_surrogate_model()
            except Exception as e:
                logger.warning(f"Failed to update CBO surrogate model: {e}")
    ...
    def refine_optimization_candidates(self, best_results: pd.DataFrame):
        ...
        from llmservingtuner.optimizer.experience_fine_tunning import StopFineTune
        # 分别精调每组参数
    ...
    def prepare_plugin(self):
        from llmservingtuner.config.config import get_settings, field_to_param
        from llmservingtuner.config.model_config import MindieModelConfig
        from llmservingtuner.optimizer.plugins.benchmark import AisBench
        from llmservingtuner.optimizer.plugins.simulate import Simulator, DisaggregationSimulator
    ...
    def run_plugin(self):
        from llmservingtuner.config.config import map_param_with_value, get_settings
        from llmservingtuner.optimizer.global_best_custom import CustomGlobalBestPSO
        self.prepare_plugin()

        # Initialize CBO surrogate model if enabled
        if self.cbo_enabled:
            try:
                settings = get_settings()
                from llmservingtuner.optimizer.cbo_surrogate_model import StagedCBO
                self.cbo_model = StagedCBO(
                    param_dim=self.dimensions(),
                    bounds=self.constructing_bounds(),
                    objective_index=0,
                    constraint_thresholds=self.constraint_thresholds,
                    initial_points=settings.cbo.initial_points,
                    strategy=settings.cbo.strategy,
                    feasibility_threshold=settings.cbo.feasibility_threshold,
                )
                logger.info("CBO surrogate model initialized successfully")
            except ImportError as e:
                logger.warning(f"CBO dependencies not available: {e}. Running without surrogate model.")
                self.cbo_enabled = False
                self.cbo_model = None
            except Exception as e:
                logger.warning(f"Failed to initialize CBO model: {e}. Running without surrogate model.")
                self.cbo_enabled = False
                self.cbo_model = None
...
def plugin_main(args: argparse.Namespace):
    from llmservingtuner.optimizer.server import main as slave_server
    from llmservingtuner.optimizer.store import DataStorage
    from llmservingtuner.config.config import get_settings
    from llmservingtuner.optimizer.experience_fine_tunning import FineTune
    from llmservingtuner.optimizer.scheduler import Scheduler
    from llmservingtuner.optimizer.plugins.simulate import DisaggregationSimulator
    from llmservingtuner.optimizer.register import register_ori_functions
    ...
    # CBO configuration
    cbo_enabled = settings.cbo.enabled
    try:
...
def arg_parse(subparsers):
    from llmservingtuner.plugins import load_general_plugins
