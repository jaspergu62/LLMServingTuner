...
from llmservingtuner.common import get_train_sub_path, is_mindie, is_vllm
from llmservingtuner.config.base_config import REAL_EVALUATION
from llmservingtuner.config.config import get_settings, PerformanceIndex, OptimizerConfigField, \
    map_param_with_value, CommunicationConfig, Stage
from llmservingtuner.config.base_config import FOLDER_LIMIT_SIZE, REQUESTRATES
from llmservingtuner.optimizer.communication import CommunicationForFile, CustomCommand
...
from llmservingtuner.optimizer.utils import get_folder_size
...
class Scheduler:
    ...
    def save_dummy_result(self, is_predicted: bool = True, **kwargs):
        """
        Save a dummy/predicted result without running actual evaluation.
        Used by CBO surrogate model for filtered candidates.
        """
        self.data_storage.save(
            None,  # No performance index for predicted results
            tuple(self.simulate_run_info) if self.simulate_run_info else tuple(),
            error=None,
            backup=None,
            duration=0,
