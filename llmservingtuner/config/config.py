...
class CBOConfig(BaseModel):
    """Configuration for Constrained Bayesian Optimization (CBO) surrogate model."""
    enabled: bool = True  # Whether to use CBO surrogate model (default: enabled)
    initial_points: int = Field(default=5, ge=2, le=50)  # Points before activating surrogate
    strategy: str = "conservative"  # Selection strategy
    feasibility_threshold: float = Field(default=-1.2)  # Log probability threshold
...
class Settings(BaseSettings):
    ...
    cbo: CBOConfig = CBOConfig()  # CBO surrogate model configuration
