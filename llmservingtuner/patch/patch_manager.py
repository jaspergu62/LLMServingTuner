...
def add_diff_patch(target_file, patch_file):
    """Apply a diff-style patch using GNU patch command."""
    target_path = Path(target_file)
    patch_path = Path(patch_file)

    backup_file = str(target_path) + '.backup'
    if os.path.exists(backup_file):
        restore_cmd = f"cp {backup_file} {target_path}"
        logger.info(f"> {restore_cmd}")
        os.system(restore_cmd)
    else:
        backup_cmd = f"cp {target_path} {backup_file}"
        logger.info(f"> {backup_cmd}")
        os.system(backup_cmd)

    patch_cmd = f"patch --verbose {target_path} {patch_path}"
    logger.info(f"> {patch_cmd}")
    os.system(patch_cmd)


def add_replace_patch(target_file, patch_file):
    """Replace target file with patch file content (with backup)."""
    target_path = Path(target_file)
    patch_path = Path(patch_file)
    ...
    patch_cmd = f"cp {patch_path} {target_path}"
    logger.info(f"> {patch_cmd}")
    os.system(patch_cmd)
