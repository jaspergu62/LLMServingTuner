...
class ShareGPTDataset(BenchmarkDataset):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self._safe_token_prompt: Optional[str] = None
        self.load_data()
...
# -----------------------------------------------------------------------------
# Vidur Trace Dataset Implementation
# -----------------------------------------------------------------------------


class VidurTraceDataset(BenchmarkDataset):
    """
    Dataset helper for Vidur trace CSV files containing arrival and token stats.
    Expected CSV columns:
        - arrived_at (optional): request arrival timestamp in seconds.
        - num_prefill_tokens: prompt token count per request.
        - num_decode_tokens: decode token count target per request.
    """

    REQUIRED_COLUMNS = {"num_prefill_tokens", "num_decode_tokens"}

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.load_data()
        self._base_token_ids: Optional[list[int]] = None
        self._base_token_strs: Optional[list[str]] = None

    def load_data(self) -> None:
        if not self.dataset_path:
            raise ValueError("dataset_path must be provided for VidurTraceDataset.")

        if not self.dataset_path.endswith(".csv"):
            raise ValueError("Vidur trace dataset must be provided as a CSV file.")

        df = pd.read_csv(self.dataset_path)
        missing = self.REQUIRED_COLUMNS.difference(df.columns)
        if missing:
            raise ValueError(
                f"CSV file {self.dataset_path} is missing required columns: {missing}"
            )
        self.data = df


    def _randomize_prompt_ids(
        self,
        prompt_ids: list[int],
        tokenizer: PreTrainedTokenizerBase,
        ...
        if not prompt_ids:
            return
        random_span = min(8, len(prompt_ids))
        start_idx = rng.integers(0, len(prompt_ids) - random_span + 1)
        for offset in range(random_span):
            attempts = 0
            new_id = prompt_ids[start_idx + offset]
            while attempts < 10:
                candidate = int(rng.integers(low=100, high=min(vocab_size, 1000)))
                if candidate not in special_ids:
                    new_id = candidate
                    break
                attempts += 1
            prompt_ids[start_idx + offset] = new_id
    ...
    def sample(
        ...
        **kwargs,
        ...
        df = self.data.copy()
        total_rows = len(df)
        logger.info("Vidur trace contains %d rows.", total_rows)
        if num_requests > total_rows:
            logger.info(
                "Requested %d prompts but trace only has %d rows. Truncating.",
                num_requests,
                total_rows,
            )
            num_requests = total_rows

        rng = np.random.default_rng(self.random_seed)
        df = (
            df.sample(frac=1.0, random_state=self.random_seed)
            .reset_index(drop=True)
            .iloc[:num_requests]
        )

        mean_tokens_per_request = float("nan")
        mean_rps = float("inf")
        mean_tps = float("inf")
        if {"num_prefill_tokens", "num_decode_tokens"}.issubset(df.columns):
            mean_tokens_per_request = (
                df["num_prefill_tokens"] + df["num_decode_tokens"]
            ).mean()
        if "arrived_at" in df:
            arrivals = df["arrived_at"].dropna().to_numpy(dtype=float)
            if len(arrivals) >= 2:
                duration = arrivals.max() - arrivals.min()
                if duration > 0:
                    mean_rps = len(arrivals) / duration
        if np.isfinite(mean_rps) and not np.isnan(mean_tokens_per_request):
            mean_tps = mean_rps * mean_tokens_per_request

        logger.info(
            "Vidur trace sampling stats: mean tokens/request=%.2f, mean RPS=%s, mean TPS=%s",
            mean_tokens_per_request,
        ...
        for row in df.itertuples(index=False):
            ...
            prefill_tokens = max(
                1,
                int(round(getattr(row, "num_prefill_tokens") * prefill_scale_factor)),
            )
            decode_tokens = max(
                1,
                int(round(getattr(row, "num_decode_tokens") * decode_scale_factor)),
            )

            if max_total_tokens is not None:
                max_prefill = max(max_total_tokens - 1, 1)
                if prefill_tokens > max_prefill:
                    prefill_tokens = max_prefill
                max_decode_budget = max_total_tokens - prefill_tokens
                if max_decode_budget < 1:
                    continue
                if decode_tokens > max_decode_budget:
                    decode_tokens = max_decode_budget

            prompt, prompt_len = self._generate_prompt_from_tokens(
                tokenizer, prefill_tokens, rng
            )
            ...
            if apply_chat_template:
                ...
                prompt_len = len(tokenizer(prompt).input_ids)
                if max_total_tokens is not None:
                    max_decode_budget = max_total_tokens - prompt_len
                    if max_decode_budget < 1:
                        continue
                    if decode_tokens > max_decode_budget:
                        decode_tokens = max_decode_budget
            ...
            arrival_time = None
            if hasattr(row, "arrived_at"):
                raw_arrival = getattr(row, "arrived_at")
                if not pd.isna(raw_arrival):
                    arrival_time = float(raw_arrival) * time_scale_factor

            samples.append(
                SampleRequest(
                    prompt=prompt,
                    prompt_len=prompt_len,
                    expected_output_len=decode_tokens,
